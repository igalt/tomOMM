  ...
  async.waterfall([
    ...
    function (token, done) {
      if (req.body.criteria) {
        var querySrting = {};
        if (req.body.criteria.indexOf('@') === -1) {
          querySrting = { username: req.body.criteria.toLowerCase() };
        } else {
          querySrting = { email: req.body.criteria.toLowerCase() };
        }
        User.findOne(querySrting, '-salt -password', function (err, user) {
          if (!user) {
