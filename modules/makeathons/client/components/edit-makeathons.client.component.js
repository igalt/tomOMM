(function () {
  ...
  app.component('makeathonsEditComponent', {
    bindings: {
      makeathon: '='
    },
    templateUrl: 'modules/makeathons/client/views/view-makeathons.client.view.html',
    controller: makeathonsEditComponent,
    controllerAs: 'vm'
  });

  makeathonsEditComponent.$inject = ['$scope', '$state', '$filter', 'Authentication',
                                  'MakeathonsUtils', 'FileUploader', 'SendMailService', 'CommonService', '$window'];


  function makeathonsEditComponent ($scope, $state, $filter, Authentication,
                                      MakeathonsUtils, FileUploader, SendMailService, CommonService, $window) {
    ...
    // Check if manage mode
    function manageMode(){
      return ($state.current.name.split('.')[0] === 'manage');
    }

    // Check if edit mode
    if ($state.current.name === 'manage.makeathonEdit'){
      logger('info', 'Makeathon edit');
      vm.makeathonFormEditable = true;
      vm.uploader.url = 'api/makeathons/' + vm.makeathon._id;
    ...
    function prepareToSave(){
      ...
      console.log('Saving ', vm.makeathon);
    }
